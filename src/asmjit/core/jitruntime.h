...
public:
  ...
  template<typename tsjitFunc>
  inline Error add(tsjitFunc* dst, CodeHolder* code) noexcept {
    return _add(Support::ptr_cast_impl<void**, tsjitFunc*>(dst), code);
  }

  //! Releases `p` which was obtained by calling `add()`.
  template<typename tsjitFunc>
  inline Error release(tsjitFunc p) noexcept {
    return _release(Support::ptr_cast_impl<void*, tsjitFunc>(p));
  }
